...
4. 开源项目优先选今日涨星快的和 Agent 相关的，至少 8 个
5. 落地风向标至少 6 条，包含融资、产品上线、企业落地案例
6. 大佬说了啥至少 4 条，优先选有争议或有洞见的观点
7. Agent 动态至少 4 条
8. 趋势洞察至少 3 条
9. 如果原始内容中某板块信息不足，用当前 AI 领域真实的近期动态补充
10. JSON 必须合法，不要有多余的逗号或注释
11. 所有字符串值中不能包含换行符，请用空格代替"""

try:
    resp = requests.post(
        "https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions",
        headers={"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"},
        json={"model": "qwen-plus", "messages": [{"role": "user", "content": prompt}], "max_tokens": 4000},
        timeout=120
    )
    result = resp.json()
    ...
    data = {
        ...
        "boss_says": [], "open_source": [], "landing": [], "agent_news": [],
...
def must_cards(items):
    html = ""
    for item in items:
        html += f'<div class="must-card"><div class="must-rank">{item.get("rank","")}</div><div class="must-title">{item.get("title","")}</div><div class="must-why">{item.get("why","")}</div><div class="must-tags">{tags_html(item.get("tags",[]))}</div></div>'
    return html
...
def boss_cards(items):
    ...
    for i, item in enumerate(items):
        ...
        html += f'<div class="boss-card"><div class="boss-header"><div class="boss-avatar" style="background:{bg};color:{fg}">{item.get("initials","AI")}</div><div class="boss-info"><div class="boss-name">{item.get("name","")}</div><div class="boss-handle">{item.get("role","")}</div></div></div><div class="boss-quote">{item.get("quote","")}</div><div class="boss-comment">{item.get("comment","")}</div></div>'
    return html
...
def open_cards(items):
    ...
    for item in items:
        ...
        html += f'<div class="open-card"><div class="open-header"><div class="open-name">{item.get("name","")}</div><div class="open-right">{hot_html}<span class="open-stars">{item.get("stars","")}</span></div></div><div class="open-desc">{item.get("desc","")}</div><div class="open-meta"><span class="lang-badge">{item.get("lang","")}</span><span class="open-use">{item.get("use_case","")}</span></div></div>'
    return html

def landing_cards(items):
    html = ""
    for item in items:
        html += f'<div class="fund-card"><div class="fund-amount">{item.get("amount","")}<span>{item.get("type","")}</span></div><div class="fund-content"><div class="fund-company">{item.get("company","")}</div><div class="fund-desc">{item.get("desc","")}</div></div></div>'
    return html

def agent_cards(items):
    html = ""
    for item in items:
        html += f'<div class="agent-card"><div class="agent-badge">{item.get("badge","")}</div><div class="agent-title">{item.get("title","")}</div><div class="agent-desc">{item.get("desc","")}</div></div>'
    return html
...
<div class="page">
<div class="header">
<div class="header-top"><div class="logo">AI 科技日报</div><div class="date-badge">{today} {wd}</div></div>
<div class="subtitle">每天 08:00 自动更新 · AI 精选</div>
</div>
<div class="section">
<div class="section-header"><div class="section-icon icon-must">⚡</div><div class="section-title">今日必知</div><div class="section-count">{len(data.get('must_know',[]))} 条</div></div>
{must_cards(data.get('must_know',[]))}
</div>
<div class="section">
<div class="section-header"><div class="section-icon icon-boss">💬</div><div class="section-title">大佬说了啥</div><div class="section-count">{len(data.get('boss_says',[]))} 条</div></div>
{boss_cards(data.get('boss_says',[]))}
</div>
<div class="section">
<div class="section-header"><div class="section-icon icon-open">🔧</div><div class="section-title">热门开源项目</div><div class="section-count">{len(data.get('open_source',[]))} 个</div></div>
{open_cards(data.get('open_source',[]))}
</div>
<div class="section">
<div class="section-header"><div class="section-icon icon-fund">💰</div><div class="section-title">落地风向标</div><div class="section-count">{len(data.get('landing',[]))} 条</div></div>
{landing_cards(data.get('landing',[]))}
</div>
<div class="section">
<div class="section-header"><div class="section-icon icon-agent">🤖</div><div class="section-title">Agent 动态</div><div class="section-count">{len(data.get('agent_news',[]))} 条</div></div>
{agent_cards(data.get('agent_news',[]))}
</div>
<div class="section">
<div class="section-header"><div class="section-icon icon-insight">🔮</div><div class="section-title">趋势洞察</div><div class="section-count">{len(data.get('insights',[]))} 条</div></div>
{insight_cards(data.get('insights',[]))}
</div>
<div class="footer"><div class="footer-text">每天 08:00 自动更新 · maxwin7373-wq.github.io/ai-daily-report</div></div>
</div>
</body>
</html>"""
